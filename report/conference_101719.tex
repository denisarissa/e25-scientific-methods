...
Based on a criteria list, related papers were found by using the selected search keywords as mentioned in the methodology chapter. The criteria items used to select papers were as following:

\begin{itemize}
\item Multi drone
\item Describes type data sent and received
\item Describes type of communication (broadcast, one-to-one)
\item Describes architecture layer
\item Specifies mission context (search and rescue, transporting of goods)
\item In English and from last 10 years
\end{itemize}

The papers are from the last 10 years and there is a notable concentration in recent years which aligns with the growing interest in multi-drone systems.
